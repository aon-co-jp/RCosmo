...
6. Cargo.lock を open-cosmo 直下に生成（バージョン固定）。
...
1. **KeyGuardian（API キー完全自動運用）**（`router::keyring`）:
   ...
   （応答の `urn:open-cosmo:params:scim:api-key` に一度だけ平文表示、保存は SHA-256）。
...
open-cosmo は以下の全サブプロジェクトのバックボーンとして機能する:
- **open-e-gov** (電子政府)
...
└── docs/
    ...
    ├── why-open-cosmo.md          ← REST API との比較表
    └── HANDOFF.md               ← 本ファイル
```
...
6. **open-cosmo を中心ミドルウェア化するアーキテクチャ更新**  
   `docs/architecture.md` に open-cosmo が open-e-gov / OpenRedmine / OpenWordPress の  
...
| 判断 | 理由 |
|------|------|
| 名称を OpenCosmo → open-cosmo に変更 | 商標問題の懸念 |
