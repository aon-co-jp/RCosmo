...
open-cosmo is the **backbone of the entire open-aruaru project family**.
Every product-level subproject talks to its subgraph services *through*
open-cosmo — never directly.
...
- Subprojects (open-e-gov, OpenRedmine, OpenWordPress) publish their schemas
  to the Schema Registry; open-cosmo composes them into one federated graph.
